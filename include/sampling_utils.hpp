...
public:
  ...
  Vector getDiskSample() const {
    float phi = getFloatSample(0.0f, toolbox::DOUBLE_PI);
    return Vector(cosf(phi), sinf(phi));
  }

  Vector getSemidiskSample() const {
    float phi = getFloatSample(0.0f, toolbox::PI);
    return Vector(cosf(phi), sinf(phi));
  }
  ...
  Vector getCosineLobeSample(float cosPower) const {
    float r1 = getFloatSample(0.0f, toolbox::DOUBLE_PI);
  ...
  Vector getNormalSample(float mu = 0.0f, float sigma = 1.0f) {
    float u1 = getFloatSample(EPSILON, 1.0f);
    float u2 = getFloatSample();

    float r = sqrtf(-2.0f * lnf(u1));
    float theta = DOUBLE_PI * u2;

    float z0 = r * cosf(theta);
    float z1 = r * sinf(theta);

    return Vector(mu + z0 * sigma, mu + z1 * sigma);
  }
};
